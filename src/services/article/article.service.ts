...
export class ArticleService extends TypeOrmCrudService<Article>{
    ...
    async editFullArticle(articleId: number, data: EditArticleDto): Promise <Article | ApiResponse>{
        ...
        const lastPriceStirng: string = Number(lastPrice).toFixed(2);
        
        if(newPriceString !== lastPriceStirng){
            ...
            if(!savedArticlePrice){
                return new ApiResponse('error',-5003, 'Could not saved new article price')
            }
        }
    ...
    async search(data: ArticleSearchDto): Promise<Article[] | ApiResponse>{
        ...
        if(articles.length === 0){
            return new ApiResponse('ok',0,'No articles found for these search parametars!');
        }
